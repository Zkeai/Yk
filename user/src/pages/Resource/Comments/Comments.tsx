...
import '../index.less'
import {
  addComments, deleteComments, editComments,
searchComGroup, searchComments,
...
const App = () => {
  ...
  useEffect(() => {
    getComGroupList().then()
  },[])

  const wordKeyDelete = async (value: API.keySearch) => {
    const res = await deleteComments({'id':value.id,"keyWord":value.keyWord})
    if(res.code === 0){
  ...
  const onFinish =async (values: any) => {
    ...
    const res = await addComments({keyWord,keyGroup,note,content,status})
    if(res.code === 0 && res.data !== ""){
      setIsModalVisible(false)
      setWordKey(wordKey + 1);
      message.success("术语添加成功",2)
    }else{
  ...
  const EditFinish = async (value: any)=>{
    const res = await editComments({...value})
    if(res.code === 0){
      setWordKey(wordKey + 1)
      setEditIsModalVisible(false)
  ...
  return(
    ...
    <>
      ...
      <Modal footer={null} title="添加术语" visible={isModalVisible}  onCancel={handleCancel}>
        <Form
          name="addKeyGroup"
          labelCol={{ span: 8 }}
          wrapperCol={{ span: 16 }}
      ...
      <Modal footer={null} title="修改术语" visible={isEditModalVisible}  onCancel={handleEditCancel}>
      ...
      <Table rowKey={record => record.id} style={{marginTop:"10px"}} loading={hasLoading}  columns={columns} dataSource={Comments} rowSelection={rowSelection} />;
    </>
  )
